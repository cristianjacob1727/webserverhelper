...
function deleteTargets( $repositoryHome  )
{
  $output = "";
  $currentDir = getcwd();
  chdir( $repositoryHome );

  $dirs = array_filter(glob('*'), 'is_dir');

  foreach ($dirs as $dir) {

    $subDir = $repositoryHome . __bslash() . __bslash() . $dir;
    if ($dir === "target") {
      //$output .= 'deleting directory ' . $subDir . endl();
      deleteDirectory( $subDir );
      break;
    }
    deleteTargets( $subDir );
...
if (isset( $_POST['action'] )) {
  switch ($_POST['action']) {
    ...
    case 'set_config':
      ...
      break;     
    case 'delete_targets':
      deleteTargets( $appConfig->repository_home );
      sendResponse('delete_targets');
      break;
